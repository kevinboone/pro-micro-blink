# 
# Makefile for the trivial "blink" example for Pro Micro. This could serve
# as a Makefile for more complex examples: just add the object files
# that comprise the program to PROG_OBJS. We assume that each of these
# objects is generated from a C++ source file with extension .cpp.
#
# We could work in C instead, by modifying the complilation commands
# and file extensions, but there's little point -- many of the stock
# Arduino libraries only work (sigh) with C++ source.
#
# Because the Pro Micro is somewhat non-standard, it has its own set
# of headers that have to take precedence over the standard headers.
# It's easiest (in my view) to get the necessary bits by installing
# Pro Micro support in the Arduino IDE, and then specifying the locations
# of the headers in this Makefile using a -I switch. It's also necessary
# to provide the locations of the standard Arduino headers, but the
# custom headers have to come first.
#
...
# Pro Micro has this name, followed by ".hex".
NAME=blink
...
$(NAME).elf: $(PROG_OBJS) $(LIB_CPP_OBJS) $(LIB_C_OBJS)
	$(CC) $(LDFLAGS) -o blink.elf $(PROG_OBJS) $(LIB_CPP_OBJS) $(LIB_C_OBJS)
